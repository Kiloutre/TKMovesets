...
std::string EditorT8::GetCommandStr(uint64_t command) const
{
	return getCommandString(command);
}

std::string EditorT8::GetCommandStr(const char* commandBuf) const
{
	uint64_t command = (uint64_t)strtoll(commandBuf, nullptr, 16);
	return getCommandString(command);
}

std::string EditorT8::GetCommandStr(const char* direction, const char* button) const
{
...
void EditorT8::GetInputSequenceString(int id, std::string& outStr, int& outSize) const
{
	std::string retVal;
