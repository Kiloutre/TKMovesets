		...
		for (int i = 0; str[i]; ++i)
		{
			if (str[i] == '{')
			{
				...
				if (str[i] == '}') {
					...
					if (!found)
					{
						for (auto& [key, value] : EditorT8Utils::mvlDisplay_characterConversion)
						{
...
uint64_t EditorT8::CreateMoveName(const char* moveName)
{
...
void EditorT8::SetupIterators_DisplayableMovelist()
{
...
void EditorT8::LoadMovesetPtr(Byte* t_moveset, uint64_t t_movesetSize)
{
...
bool EditorT8::LoadMoveset(Byte* t_moveset, uint64_t t_movesetSize)
{
	constants = {
...
void EditorT8::RecomputeDisplayableMoveFlags(uint16_t moveId)
{
...
void EditorT8::ReloadDisplayableMoveList()
{
...
uint16_t EditorT8::GetCurrentMoveID(uint8_t playerId) const
{
...
void EditorT8::SetCurrentMove(uint8_t playerId, gameAddr playerMoveset, size_t moveId)
{
	gameAddr playerAddress = m_game.ReadPtrPath("p1_addr");
	if (playerId > 0) {
		playerAddress += playerId * m_game.GetValue("playerstruct_size");
...
void EditorT8::DeleteAnimationIfUnused(uint64_t anim_addr, uint64_t anim_name_addr)
{
...
std::string EditorT8::ImportAnimation(const wchar_t* filepath, int moveid)
{
	// Keep file name only
...
uint32_t EditorT8::CalculateCRC32()
{
