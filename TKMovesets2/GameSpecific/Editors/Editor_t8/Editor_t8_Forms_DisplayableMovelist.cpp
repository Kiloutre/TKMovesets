...
std::string EditorT8::GetMovelistDisplayableText(uint32_t offset)
{
	...
	for (int i = 0; i < maxLen;)
	{
		...
		{
			auto entry = EditorT8Utils::mvlDisplay_characterConversion.find((unsigned char)entryString[i + 2]);

			if (entry != EditorT8Utils::mvlDisplay_characterConversion.end()) {
...
std::string EditorT8::GetMovelistDisplayableLabel(InputMap& fieldMap) const
{
	std::string retVal;
...
std::vector<InputMap> EditorT8::GetMovelistInputListInputs(uint16_t id, int listSize, VectorSet<std::string>& drawOrder)
{
...
void EditorT8::SaveMovelistInput(uint16_t id, InputMap& inputs)
{
	auto input = m_iterators.mvl_inputs.begin() + id;
...
std::vector<InputMap> EditorT8::GetMovelistDisplayablesInputs(uint16_t id, VectorSet<std::string>& drawOrder, bool singleItem)
{
	std::vector<InputMap> inputListMap;
...
void EditorT8::SaveMovelistDisplayable(uint16_t id, InputMap& inputs)
{
	auto displayable = m_iterators.mvl_displayables[id];
	...
	for (int i = 0; i < _countof(displayable->title_translation_offsets); ++i) {
		...
		std::string key = "title_translation_" + std::to_string(i);
		std::string convertedBuffer = EditorT8Utils::ConvertMovelistDisplayableTextToGameText(inputs[key]->buffer);
...
bool EditorT8::ValidateMovelistDisplayableField(EditorInput* field)
{
...
InputMap EditorT8::GetMovelistPlayableInputs(uint16_t id, VectorSet<std::string>& drawOrder)
{
	InputMap inputMap;
...
void EditorT8::SaveMovelistPlayable(uint16_t id, InputMap& inputs)
{
	auto playable = m_iterators.mvl_playables[id];
...
bool EditorT8::ValidateMovelistPlayableField(EditorInput* field)
{
	auto& name = field->name;
...
std::string EditorT8::GetDisplayableMovelistInputStr(const char* directions, const char* buttons) const
{
	std::string retVal;
...
int EditorT8::GetDisplayableMovelistEntryColor(EditorInput* field) const
{
...
bool EditorT8::IsMovelistDisplayableEntryCategory(EditorInput* field)const
{
	MvlDisplayableType type = (uint32_t)GetFieldValue(field);
	return type == 8;
}

bool EditorT8::IsMovelistDisplayableEntryCombo(EditorInput* field) const
{
	MvlDisplayableType type = (uint32_t)GetFieldValue(field);
	return type == 10;
}
