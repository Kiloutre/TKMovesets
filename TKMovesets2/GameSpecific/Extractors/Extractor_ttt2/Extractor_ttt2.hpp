...
class ExtractorTTT2 : public Extractor
{
private:
	/// Main functions

	// Fill the moveset table & its corresponding offset table brother.
	void FillMovesetTables(gameAddr movesetAddr, StructsTTT2::MovesetTable* table, StructsTTT2::MovesetTable* offsets);
	...
	Byte* CopyMotaBlocks(gameAddr movesetAddr, uint64_t& size_out, StructsTTT2::MotaList* motasList, ExtractSettings settings);
	...
	Byte* CopyAnimations(const StructsTTT2::Move* movelist, size_t moveCount, uint64_t& size_out, std::map<gameAddr, uint64_t>& offsets);
	// Copies the moveset info block (aliases, couple strings)
	void CopyMovesetInfoBlock(gameAddr movesetAddr, StructsTTT2::MovesetInfo* movesetHeader);
	...
	void GetNamesBlockBounds(const StructsTTT2::Move* move, uint64_t moveCount, gameAddr& start, gameAddr& end);
