...
uint64_t ExtractorT6::CalculateMotaCustomBlockSize(const MotaList* motas, std::map<gameAddr32, std::pair<uint32_t, uint32_t>>& offsetMap, ExtractSettings settings)
{
...
Byte* ExtractorT6::AllocateMotaCustomBlock(MotaList* motas, uint64_t& size_out, ExtractSettings settings)
{
...
Byte* ExtractorT6::CopyMovesetBlock(gameAddr movesetAddr, uint64_t& size_out, const MovesetTable& table, const MovesetTable* offsets)
{
...
ExtractionErrcode_ ExtractorT6::Extract(gameAddr playerAddress, ExtractSettings settings, uint8_t& progress)
{
	progress = 0;
...
std::string ExtractorT6::GetPlayerCharacterName(gameAddr playerAddress)
{
	...
	m_process->readBytes(movesetAddr + 0x270, characterName, sizeof(characterName));
