...
static bool ConvertT5DRToT7(const movesetInfo& mInfo, const TKMovesetHeader* orig_header, Byte* orig_moveset, uint64_t s_moveset)
{
	StructsT7::TKMovesetHeaderBlocks blocks;
	...
	{
		if (!MovesetConverter::T5DRToT7().Convert(orig_header, orig_moveset, s_moveset, blocks)) {
			return false;
		}
	}
	...
	new_header->gameId = GameId_T7;
	new_header->minorVersion = GameVersions::T7::NONE;
	new_header->conversion_origin = GameVersions::T7::FROM_T5DR;
	...
	switch (targetGameId)
	{
		case GameId_T7:
		{
			switch (mInfo.gameId)
			{
				...
				case GameId_T5:
				{
					if (mInfo.minorVersion == GameVersions::T5::DR_ONLINE) {
						ConvertT5DRToT7(mInfo, header, moveset, s_moveset);
					}
					else {
