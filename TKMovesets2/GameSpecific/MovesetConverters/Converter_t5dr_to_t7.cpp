...
static Byte* AllocateMovesetArea(const TKMovesetHeader* header, Byte* moveset, uint64_t s_moveset, uint64_t& size_out, TKMovesetHeaderBlocks& blocks_out)
{
	...
	auto movesetInfo = (T5DR::MovesetInfo*)moveset;
...
bool MovesetConverter::T5DRToT7::Convert(const TKMovesetHeader* header, Byte*& moveset, uint64_t& s_moveset, TKMovesetHeaderBlocks& blocks_out)
{
	...
	auto old_movesetInfo = (T5DR::MovesetInfo*)moveset;
	auto new_movesetInfo = (MovesetInfo*)new_moveset;

	memcpy(new_movesetInfo, old_movesetInfo, 8);
	new_movesetInfo->character_name_addr = (char*)0 + old_movesetInfo->character_name_addr + sizeof(MovesetInfo) - sizeof(T5DR::MovesetInfo);
	new_movesetInfo->character_creator_addr = (char*)0 + old_movesetInfo->character_creator_addr + sizeof(MovesetInfo) - sizeof(T5DR::MovesetInfo);
	new_movesetInfo->date_addr = (char*)0 + old_movesetInfo->date_addr + sizeof(MovesetInfo) - sizeof(T5DR::MovesetInfo);
	new_movesetInfo->fulldate_addr = (char*)0 + old_movesetInfo->fulldate_addr + sizeof(MovesetInfo) - sizeof(T5DR::MovesetInfo);
