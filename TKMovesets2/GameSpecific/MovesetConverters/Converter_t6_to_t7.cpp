...
{
	auto offsets = (const T6::TKMovesetHeaderBlocks*)((char*)header + header->block_list);

	auto movesetInfo = (T6::MovesetInfo*)moveset;
	...
	movesetSize += offsets->GetBlockSize(T6::TKMovesetHeaderBlocks_Animation, s_moveset);
	blocks_out.motaBlock = movesetSize;
	movesetSize += offsets->GetBlockSize(T6::TKMovesetHeaderBlocks_Mota, s_moveset);
...
bool MovesetConverter::T6ToT7::Convert(const TKMovesetHeader* header, Byte*& moveset, uint64_t& s_moveset, TKMovesetHeaderBlocks& blocks_out)
{
	...
	auto old_blocks = (const T6::TKMovesetHeaderBlocks*)((char*)header + header->block_list);
	auto old_movesetInfo = (T6::MovesetInfo*)moveset;
	auto new_movesetInfo = (MovesetInfo*)new_moveset;

	memcpy(new_movesetInfo, old_movesetInfo, 8);
	new_movesetInfo->character_name_addr = (char*)0 + old_movesetInfo->character_name_addr + sizeof(MovesetInfo) - sizeof(T6::MovesetInfo);
	new_movesetInfo->character_creator_addr = (char*)0 + old_movesetInfo->character_creator_addr + sizeof(MovesetInfo) - sizeof(T6::MovesetInfo);
	new_movesetInfo->date_addr = (char*)0 + old_movesetInfo->date_addr + sizeof(MovesetInfo) - sizeof(T6::MovesetInfo);
	new_movesetInfo->fulldate_addr = (char*)0 + old_movesetInfo->fulldate_addr + sizeof(MovesetInfo) - sizeof(T6::MovesetInfo);
	...
	for (unsigned int i = 0; i < table.reactionsCount; ++i)
	{
		...
		const T6::Reactions* source = (T6::Reactions*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.reactions) + i;
	...
	for (unsigned int i = 0; i < table.pushbackExtradataCount; ++i)
	{
		...
		const T6::PushbackExtradata* source = (T6::PushbackExtradata*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.pushbackExtradata) + i;
	...
	for (unsigned int i = 0; i < table.pushbackCount; ++i)
	{
		...
		const T6::Pushback* source = (T6::Pushback*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.pushback) + i;
	...
	for (unsigned int i = 0; i < table.requirementCount; ++i)
	{
		...
		const T6::Requirement* source = (T6::Requirement*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.requirement) + i;
	...
	for (unsigned int i = 0; i < table.cancelExtradataCount; ++i)
	{
		...
		const T6::CancelExtradata* source = (T6::CancelExtradata*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.cancelExtradata) + i;
	...
	for (unsigned int i = 0; i < table.moveCount; ++i)
	{
		...
		target->_0x98_int = T6_T7_Aliases::ConvertMove0x98(source->_0x98_int);
		target->hitbox_location = source->hitbox_location;
		T6_T7_Aliases::ApplyHitboxAlias(target->hitbox_location);
	...
	for (unsigned int i = 0; i < table.cancelCount; ++i)
	{
		...
		const T6::Cancel* source = (T6::Cancel*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.cancel) + i;
	...
	for (unsigned int i = 0; i < table.groupCancelCount; ++i)
	{
		...
		const T6::Cancel* source = (T6::Cancel*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.groupCancel) + i;
	...
	for (unsigned int i = 0; i < table.hitConditionCount; ++i)
	{
		...
		const T6::HitCondition* source = (T6::HitCondition*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.hitCondition) + i;
	...
	for (unsigned int i = 0; i < table.voiceclipCount; ++i)
	{
		...
		const T6::Voiceclip* source = (T6::Voiceclip*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.voiceclip) + i;
	...
	for (unsigned int i = 0; i < table.inputCount; ++i)
	{
		...
		const T6::Input* source = (T6::Input*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.input) + i;
	...
	for (unsigned int i = 0; i < table.inputSequenceCount; ++i)
	{
		...
		const T6::InputSequence* source = (T6::InputSequence*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.inputSequence) + i;
	...
	for (unsigned int i = 0; i < table.cameraDataCount; ++i)
	{
		...
		const T6::CameraData* source = (T6::CameraData*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.cameraData) + i;
	...
	for (unsigned int i = 0; i < table.unknownParryRelatedCount; ++i)
	{
		...
		const T6::UnknownParryRelated* source = (T6::UnknownParryRelated*)(old_blocks->GetBlock(T6::TKMovesetHeaderBlocks_Moveset, moveset) + old_movesetInfo->table.unknownParryRelated) + i;
